...
class DoserBot:
    def __init__(self, definitions: dict, serial_port: Union[str, None] = None, queue_size: int = 10):
        ...
        self.load_cell_status_dict = {}

        # Message types
        self.motor_status_message_id = self.definitions_dict['message_types']['MOTOR_STATUS_MESSAGE_ID']
        self.motor_pulses_message_id = self.definitions_dict['message_types']['MOTOR_PULSES_MESSAGE_ID']
        self.load_cell_status_message_id = self.definitions_dict['message_types']['LOAD_CELL_STATUS_MESSAGE_ID']
        ...
        # Who am I
        self.who_am_i_message_id = self.definitions_dict['message_types']['WHO_AM_I_MESSAGE_ID']
        self.who_am_i_buffer = struct.pack('!4B', self.STX, 4, self.definitions_dict['who_am_i']['WHO_AM_I'], self.ETX)

        # Motors
        self.motors_in_use = []
        self.motor_ids_in_use = []

        # Load cells
        self.load_cells_in_use = []
        self.load_cell_ids_in_use = []

        self.microsteps = [1, 2, 4, 8, 16, 32]
        max_pulses_per_second = 1e6 / self.definitions_dict['motors']['MINIMUM_PULSE_INTERVAL_uS']

        for i in range(self.definitions_dict['motors']['NUMBER_OF_MOTORS']):
            if self.definitions_dict[f'motor_{i}']['in_use'] != 0:
                self.motors_in_use.append(i)
                motor_id = self.definitions_dict[f'motor_{i}']['id']
                self.motor_ids_in_use.append(motor_id)
                self.motor_status_dict[f'motor_{motor_id}_status'] = 0
                self.motor_status_dict[f'motor_{motor_id}_job_id'] = 0
                self.motor_status_dict[f'motor_{motor_id}_pulses_remaining'] = 0

            max_motor_rpm = (max_pulses_per_second / self.definitions_dict[f'motor_{i}']['steps_per_rev']) * 60
            self.definitions_dict[f'motor_{i}']['max_rpm'] = [max_motor_rpm/j for j in self.microsteps]

        for i in range(self.definitions_dict['load_cell']['NUMBER_OF_LOADCELLS']):
            if self.definitions_dict[f'load_cell_{i}']['in_use'] != 0:
                self.load_cells_in_use.append(i)
                load_cell_id = self.definitions_dict[f'load_cell_{i}']['id']
                self.load_cell_ids_in_use.append(load_cell_id)
                self.load_cell_status_dict[f'load_cell_{load_cell_id}_value'] = 0

        # Incoming messages don't include header bytes
        self.motor_status_message_struct = struct.Struct('!22B')
        self.motor_pulses_message_struct = struct.Struct('!B10lB')
